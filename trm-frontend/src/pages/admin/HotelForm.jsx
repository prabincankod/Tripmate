...
const HotelForm = ({ formData, setFormData, handleSubmit, places }) => {
  ...
  const handleInputChange = (e) => {
    ...
    // 10-digit validation for contact
    if (name === "contact") {
      if (!/^\d{0,10}$/.test(value)) return; // Only allow digits up to 10
  ...
  return (
    ...
    >
      ...
      {/* Contact */}
      <div>
        <label className="block font-medium mb-1">Contact (10 digits) *</label>
        <input
          type="text"
          name="contact"
      ...
      <div>
        ...
        />
        {formData.image && (
          <div className="w-32 h-32 mt-2 overflow-hidden rounded border">
            <img
              src={
        ...
        {formData.roomFeatures.map((item, idx) => (
          <div key={idx} className="flex gap-2 mb-1">
            <input
              type="text"
              value={item}
              onChange={(e) => handleArrayChange("roomFeatures", idx, e.target.value)}
              className="border p-1 rounded flex-1"
            />
            <button
              type="button"
